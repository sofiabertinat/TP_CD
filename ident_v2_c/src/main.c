...
void receiveData (float* buffer)
{
   ...
   u_i = (int16_t) (((float)dacValue/1023.0)*100); // multiplicamos por 100 para no perder la parte decimal
   //uartWriteByteArray ( UART_USB ,(uint8_t* )(&u_i),sizeof(int16_t));
...
void m_console_print (float* buffer)
{
   ...
   int32_t separador = -1;
	...
	// Imprimo los parámetros calculados
   uartWriteByteArray ( UART_USB ,(uint8_t* )(&separador),sizeof(int32_t));
	for (i = 0; i<5; i++)
	{
